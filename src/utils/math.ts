/**
 * Mathematical utilities, procedural noise, and color interpolation
 */
...
export class PRNG {
  ...
  constructor(seed: number) {
    this.state = Math.floor(seed) || 123456789;
  }
...
// 2D/3D Simplex / Perlin Noise Generator
export class NoiseGenerator {
  ...
  // Curl Noise in 2D (Incompressible vector field gradient)
  curl2D(x: number, y: number, eps = 0.001): { dx: number; dy: number } {
    const n1 = this.noise2D(x, y + eps);
    const n2 = this.noise2D(x, y - eps);
    const n3 = this.noise2D(x + eps, y);
    const n4 = this.noise2D(x - eps, y);
...
// Color conversion and interpolation functions
...
// Sample a smooth gradient across multiple hex colors at normalized t [0, 1]
export function sampleGradient(colors: string[], t: number): string {
  if (!colors || colors.length === 0) return '#ffffff';
  if (colors.length === 1) return colors[0];

  const clampedT = Math.max(0, Math.min(1, ((t % 1) + 1) % 1));
